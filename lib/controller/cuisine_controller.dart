...
import '/data/model/response/cuisine_restaurants_model.dart';
...
class CuisineController extends GetxController implements GetxService {
  ...
  Future<void> getCuisineRestaurantList(
    ...
    Response response =
        await cuisineRepo.getCuisineRestaurantList(offset, cuisineId);
    if (response.statusCode == 200) {
      if (offset == 1) {
        _cuisineRestaurantsModel =
            CuisineRestaurantModel.fromJson(response.body);
      } else {
        _cuisineRestaurantsModel!.totalSize =
            CuisineRestaurantModel.fromJson(response.body).totalSize;
        _cuisineRestaurantsModel!.offset =
            CuisineRestaurantModel.fromJson(response.body).offset;
        _cuisineRestaurantsModel!.restaurants!.addAll(
            CuisineRestaurantModel.fromJson(response.body).restaurants!);
      }
    } else {
      ApiChecker.checkApi(response);
    }
