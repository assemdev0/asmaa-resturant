...
import '/view/screens/checkout/widget/order_successfull_dialog.dart';
    ...
    if (isSuccess) {
      ...
      if (Get.find<OrderController>().paymentMethodIndex == 0 ||
        ...
        if (GetPlatform.isWeb) {
          ...
          String selectedUrl =
              '${AppConstants.baseUrl}/payment-mobile?order_id=$orderID&customer_id=${Get.find<UserController>().userInfoModel!.id}&payment_method=${Get.find<OrderController>().digitalPaymentName}&payment_platform=web&&callback=$protocol//$hostname${RouteHelper.orderSuccess}?id=$orderID&amount=$amount&status=';
