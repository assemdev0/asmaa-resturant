...
class CheckoutHelper {
  static double getDeliveryCharge(
      {required RestaurantController restController,
    ...
    double perKmCharge = restController.restaurant!.selfDeliverySystem == 1
        ? restController.restaurant!.perKmShippingCharge!
        : zoneData.perKmShippingCharge ?? 0;

    double minimumCharge = restController.restaurant!.selfDeliverySystem == 1
        ? restController.restaurant!.minimumShippingCharge!
        : zoneData.minimumShippingCharge ?? 0;

    double? maximumCharge = restController.restaurant!.selfDeliverySystem == 1
    ...
    if (restaurantSubscriptionActive) {
      if (orderController.subscriptionOrder &&
        ...
        if (orderController.subscriptionType == 'weekly') {
          ...
          subscriptionQty =
              orderController.subscriptionRange!.duration.inDays + 1;
        }
      }
    }
