...
class LocationController extends GetxController implements GetxService {
  ...
  Future<void> getAddressList({bool canInsertAddress = false}) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (canInsertAddress) {
        Get.find<RestaurantController>().insertAddresses(Get.context!, null);
      }
    } else {
      ApiChecker.checkApi(response);
    }
  ...
  Future<bool> saveUserAddress(AddressModel address) async {
    ...
    return await locationRepo.saveUserAddress(
        userAddress, address.zoneIds, address.latitude, address.longitude);
  }
